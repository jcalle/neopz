...
Physical Surface("water",1) = {s_c1_1, s_c3_1,s_c5_1};
Physical Surface("steel",2) 	  = {s_c2_1, s_c4_1} ;
Physical Surface("rock",3) = {s_c6_1} ;
Physical Line("boundary_dir", 4)   = boundary[];
Physical Line (4) -= {a_c1_4};
Physical Line("boundary_neumann", 5)   = {a_c1_4};
...
rockMaterials[] = Physical Surface(3);
...
Field[5] = MathEval;
Field[5].F = Sprintf("%g", el_size_4);

Field[6] = Restrict;
Field[6].IField = 5;
Field[6].FacesList = {Abs(rockMaterials[])};
Field[6].EdgesList = {Abs(rockMaterialsBound[])};
