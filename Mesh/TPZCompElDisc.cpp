...
void TPZCompElDisc::AddSolution(TPZVec<REAL> &qsi, TPZMaterialData &data){
    ...
    for (int is = 0; is<numbersol; is++) {
        data.sol[is].Resize(solVecSize);
        data.sol[is].Fill(0.);
        data.dsol[is].Redim(data.dphix.Rows(), solVecSize);
        data.dsol[is].Zero();
    }
    int64_t iv = 0, d;
    for(int in=0; in<ncon; in++) {
        ...
        for(int jn=0; jn<dfvar; jn++) {
            for (int64_t is=0; is<numbersol; is++) {
                ...
                for(d=0; d<data.dphix.Rows(); d++){
                    data.dsol[is](d,iv%nstate) += (STATE)data.dphix(d,iv/nstate)*MeshSol(pos+jn,is);
                }
            }
            iv++;
        }
...
REAL TPZCompElDisc::EvaluateSquareResidual2D(TPZInterpolationSpace *cel){
	...
	for(int int_ind = 0; int_ind < intrulepoints; ++int_ind){
        ...
        disc->ComputeSolution(intpoint, data);
