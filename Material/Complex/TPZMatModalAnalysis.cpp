...
{
    RealSolution(datavec,var,Solout,fEr,fEr,fUr,fUr);
}
...
void
TPZMatModalAnalysis::RealContribute(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef, STATE exx, STATE eyy,
                                    STATE ezz, STATE uxx, STATE uyy, STATE uzz) {
    /*********************CREATE H1 FUNCTIONS****************************/
    TPZFNMatrix<12,REAL> phiH1 = datavec[ h1meshindex ].phi;
    TPZFNMatrix<36,REAL> dphiH1daxes = datavec[ h1meshindex ].dphix;
    TPZFNMatrix<3,REAL> dphiH1;
...
void TPZMatModalAnalysis::RealSolution(TPZVec<TPZMaterialData> &datavec, int var, TPZVec<STATE> &Solout,
                                       STATE exx, STATE eyy, STATE uxx, STATE uyy){
    TPZVec<STATE> et(3,0.);
    TPZVec<STATE> ez(1,0.);
